...
test_that("add.resizer returns an HTML character string with correct class", {
  ...
  # Mock template.loc to point to our temporary directory
  with_mocked_env(
    env = list(
      template.loc = function(template = "themes") temp_dir
    ),
    code = {
      ...
      expect_true(grepl("default.css", resizer_output_default))
      expect_true(grepl("default_top.js", resizer_output_default))

      # Test with position = "bottom"
      resizer_output_bottom <- add.resizer(position = "bottom")
      expect_true(grepl("default_bottom.js", resizer_output_bottom))
      expect_false(grepl("default_top.js", resizer_output_bottom))
